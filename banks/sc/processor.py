...
class SCProcessor(BaseBankProcessor):
    ...
    def _parse_forward(self, email_text: str) -> pd.DataFrame:
        ...
        # Clean unicode and extract
        clean_section = re.sub(r'[^\x00-\x7F]+', ' ', forward_only)
        
        # Extract terms and rates
        lines = [line.strip() for line in clean_section.split('\n') if line.strip()]
        
        rows = []
        terms = ['1M', '3M', '6M', '9M', '12M']  # Standard terms
        
        # Extract all 5-digit rates
        all_rates = re.findall(r'\b\d{5}\b', clean_section)
        
        # Assume pairs of rates for each term (bid, ask)
        rate_index = 0
        for term in terms:
            if rate_index + 1 < len(all_rates):
                bid_rate = self._to_sc_int(all_rates[rate_index])
                ask_rate = self._to_sc_int(all_rates[rate_index + 1])
